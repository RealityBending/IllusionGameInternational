...
The initial sample consisted of `r report::report_participants(dfsub, age="Age", gender="Gender", nationality="Nationality", country="Country")`, for a total trial number of `r nrow(df)`.
...
plot_distribution(df[df$Participant %in% dfsub$Participant[1:44], ])
```
...
```{r}
outliers <- dfsub |>
  select(IPIP6_SD, IPIP6_Duration, PID5_SD, PID5_Duration) |>
  standardize(robust=TRUE) |>
  mutate(SD = rowSums(across(ends_with("SD"), ~ ifelse(is.na(.), 0, .))),
     Duration = rowSums(across(ends_with("Duration"), ~ ifelse(is.na(.), 0, .))))

table <- dfsub |>
  select(Participant, IPIP6_SD, IPIP6_Duration, PID5_SD, PID5_Duration) |>
...
dfsub <- dfsub |> 
  mutate(Age = ifelse(!is.na(Age) & !grepl("^[0-9]+$", Age), NA, as.numeric(Age)))
dfsub <- dfsub |> 
  filter(!is.na(Age))
```
...
p_age <- estimate_density(dfsub$Age) |>
  ...
  ggplot(aes(x = x)) +
  geom_histogram(data=dfsub, aes(x = Age, fill=Gender), bins=19) +
...
```{r p_nationality}
ggwaffle::waffle_iron(dfsub, ggwaffle::aes_d(group = Nationality), rows=10) |> 
  ggplot(aes(x, y, fill = group)) + 
  ggwaffle::geom_waffle() + 
  coord_equal() + 
  scale_fill_flat_d() + 
  ggwaffle::theme_waffle() +
  labs(title = "Self-reported Nationality", subtitle = "Each square represents a participant", fill="")  +
  theme(
    plot.title = element_text(size = rel(1.2), face = "bold", hjust = 0),
    plot.subtitle = element_text(size = rel(1.2)),
    axis.title.x = element_blank(),
...
The final sample includes `r report::report_participants(dfsub, age="Age", gender="Gender", nationality="Nationality", country="Country")`.
